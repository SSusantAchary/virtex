...
def create_instance_html(instance, tokenizer):
    ...
    _, image_arr = cv2.imencode(".png", image)
    image_bytes = image_arr.tobytes()
    iamge_b64 = base64.b64encode(image_bytes).decode()
...
if __name__ == "__main__":
    ...
    _A.config_override.extend(["DATA.IMAGE.COLOR_NORMALIZE", False])
    ...
    # keys: {"image_id", "image", "tokens", "masked_labels"}
    ...
    httpd = HTTPServer(("", 8765), SimpleHTTPRequestHandler)
    try:
        print(f"Serving {tempdir_path} on localhost:8765 ...")
        httpd.serve_forever()
